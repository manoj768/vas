  ...
  return (
    <div id="drr-drawer-backdrop" className="fixed inset-0 z-50 flex font-sans">
      ...
      <div
        ...
        className="relative w-80 max-w-[85%] bg-slate-900 border-r border-slate-800 text-slate-100 h-full shadow-2xl flex flex-col z-10 transform transition-transform duration-300 ease-in-out"
      >
        {/* Top User Profile Header */}
        <div className="bg-slate-950 text-slate-100 p-5 relative border-b border-slate-800">
          <button
            id="btn-close-drawer"
            onClick={onClose}
          ...
          >
            <div className="w-12 h-12 rounded-2xl bg-cyan-500/20 text-cyan-400 flex items-center justify-center shadow-md border border-cyan-500/30 group-hover:scale-105 transition-transform">
            ...
            <div>
              ...
              <p className="text-[11px] text-cyan-400 flex items-center gap-1 mt-0.5 font-bold">
          ...
          <div className="flex items-center justify-between bg-slate-900 rounded-xl px-3.5 py-2.5 border border-slate-800">
            <span className="text-xs font-bold tracking-wider uppercase text-slate-300">
              Status: <span className={currentStatus ? "text-emerald-400 font-extrabold" : "text-slate-500"}>{currentStatus ? "ON DUTY" : "OFF DUTY"}</span>
            </span>
            <button
              id="btn-toggle-duty"
              onClick={onToggleStatus}
              className={`w-11 h-6 rounded-full transition-colors relative p-0.5 ${
                currentStatus ? "bg-emerald-500" : "bg-slate-700"
              }`}
            >
              <div
          ...
          <div className="mt-3.5 bg-slate-900 border border-slate-800/90 rounded-2xl p-3.5 space-y-2.5 shadow-inner">
              ...
              <div className="w-full bg-slate-950 h-2.5 rounded-full overflow-hidden border border-slate-800 p-0.5">
                <div
                  className={`h-full rounded-full transition-all duration-300 ${
                    syncStatus === "syncing"
                      ? "bg-gradient-to-r from-cyan-500 via-teal-400 to-emerald-400 animate-pulse"
                      : pendingQueueCount > 0
                      ? "bg-amber-500"
                      : "bg-emerald-500"
                  }`}
              ...
              <div className="flex items-center justify-between text-[10px] text-slate-400 font-mono font-medium">
                <span>
                  {syncStatus === "syncing"
                    ...
                    : pendingQueueCount > 0
                    ? `${pendingQueueCount} items queued for sync`
                    : "IndexedDB up to date"}
                </span>
            ...
            <div className="pt-1.5 border-t border-slate-800/80 flex items-center justify-between text-[10px]">
              <div className="text-slate-400 truncate max-w-[170px] font-medium" title={syncProgress?.activeLabel}>
